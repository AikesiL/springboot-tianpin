<template>
    <div>
        ...
        <div class="detail-preview"
            ...
            <div class="attr"
                 :style='{"border":"1px solid #ddd","padding":"16px 16px 20px 16px","boxShadow":"0px 0px 0px #eee","margin":"20px 0 20px 0","overflow":"hidden","borderRadius":"16px","background":"#fff","position":"relative"}'>


                <el-carousel :style='{"width":"540px","margin":"0 0px 0 0","float":"right","height":"400px"}'
                             trigger="click" indicator-position="inside" arrow="always" type="default"
                             direction="horizontal" height="400px" autoplay="false" interval="3000" loop="true">
                    <el-carousel-item :style='{"borderRadius":"10px","width":"100%","height":"100%"}'
                        ...
                        <el-image :style='{"objectFit":"cover","width":"100%","height":"100%"}' v-else
                                  :src="baseUrl + item" fit="cover" class="image"></el-image>
                    </el-carousel-item>
                ...
                <div class="info"
                     :style='{"minHeight":"520px","width":"600px","padding":"0px 0 20px","margin":"0px","float":"left","background":"#fff"}'>


                    <div class="item"
                         :style='{"border":"0px solid #b0d2f2","padding":"6px 10px","boxShadow":"2px 2px 4px #87bfc2","margin":"0 0 16px 0","alignItems":"center","borderRadius":"0px","background":"radial-gradient(circle, rgba(43,224,232,1) 0%, rgba(0,173,181,1) 100%)","display":"flex","justifyContent":"space-between"}'>
                        <div :style='{"color":"#333","fontSize":"16px"}'>
                            {{detail.yongpinmingcheng}}
                        </div>
                        <div @click="storeup(1)" v-show="!isStoreup"
                             :style='{"cursor":"pointer","padding":"8px 12px","borderRadius":"20px","background":"#fff"}'>
                            <i v-if="true" :style='{"color":"#ffb966","fontSize":"14px"}'
                               class="el-icon-star-off"></i><span
                                :style='{"color":"#ffb966","fontSize":"14px"}'>点我收藏</span></div>
                        <div @click="storeup(-1)" v-show="isStoreup"
                             :style='{"cursor":"pointer","padding":"8px 12px","borderRadius":"20px","background":"#fff"}'>
                            <i v-if="true" :style='{"color":"#ffb966","fontSize":"14px"}'
                               class="el-icon-star-on"></i><span
                                :style='{"color":"#ffb966","fontSize":"14px"}'>取消收藏</span></div>
                    </div>

                    <div class="item"
                         :style='{"padding":"0px 10px","boxShadow":"0px 0px 0px #ddd","margin":"0 0 6px 0","borderColor":"#ddd","borderRadius":"0px","background":"rgba(255,255,255,.6)","borderWidth":"1px 1px 1px 1px","display":"flex","borderStyle":"dashed","justifyContent":"spaceBetween"}'
                         v-if="detail.price">
                        <div class="lable"
                             :style='{"padding":"0 10px","color":"#333","textAlign":"right","width":"130px","fontSize":"14px","lineHeight":"40px","minWidth":"130px"}'>
                            价格
                        </div>
                        <div style="font-weight: bold;"
                             :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","height":"auto"}'>
                    ...
                    <div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
                        <el-input-number
                                :style='{"width":"180px","margin":"0 5px 0 0","lineHeight":"38px","position":"relative","display":"inline-block"}'
                                v-if="detail.alllimittimes" v-model="buynumber"></el-input-number>
                        <el-button
                                :style='{"border":"1px solid #ffc800","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(241,211,87,1) 0%, rgba(254,199,0,1) 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}'
                                v-if="detail.alllimittimes" type="warning" size="small" @click="addCart">添加到购物车
                        </el-button>
                        <el-button
                                :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(245,218,167,1) 0%, rgba(255,166,0,1) 69%, rgba(255,157,0,1) 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}'
                ...
                <div class="zancai"
                     :style='{"width":"100%","padding":"0","margin":"60","justifyContent":"center","display":"flex"}'>
                    <div :style='{"margin":"20px","alignItems":"center","borderRadius":"40px","background":"#00b9ea","display":"flex","width":"150px","lineHeight":"40px","justifyContent":"center"}'
                         v-if="!isThumbsupnum && !isCrazilynum" class="zan" @click="thumbsupOrCrazily(21)">
                        <i :style='{"color":"#fff","margin":"0 3px","fontSize":"14px"}'
                ...
                <!-- 热门信息 -->
                <div class="hot"
                     :style='{"width":"100%","clear":"both","margin":"20px 0 0 0","borderRadius":"8px 8px 0 0","background":"#fff","height":"auto"}'>
                    <div :style='{"padding":"0 20px","color":"#333","borderRadius":"8px 8px 0 0","textAlign":"center","background":"radial-gradient(circle, rgba(255,236,157,1) 0%, rgba(255,200,0,1) 100%)","width":"100%","lineHeight":"44px","fontSize":"14px"}'>
                        推荐信息
                    </div>
                    <div :style='{"border":"1px solid #ffc800","padding":"20px","boxShadow":"1px 2px 3px #eee,inset 0px 0px 30px 0px #fbf5db","borderRadius":"0 0 8px 8px","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
                        <div v-for="item in hotList" :key="item"
                             :style='{"width":"23%","background":"#fff","height":"auto"}' @click="toDetail(item)">
                            <img :style='{"cursor":"pointer","width":"100%","objectFit":"cover","borderRadius":"20%","display":"block","height":"150px"}'
                                 :src="baseUrl + (item.yongpintupian?item.yongpintupian.split(',')[0]:'')" alt="">
                            <div :style='{"padding":"4px 5px 0","lineHeight":"32px","fontSize":"14px","color":"#666","textAlign":"center"}'>
            ...
            <el-tabs class="detail"
                     :style='{"border":"1px solid #eee","minHeight":"350px","boxShadow":"0px 0px 0px #ddd,inset 0px 0px 0px 0px #e0f8e8","padding":"20px","margin":"20px 0 0","borderRadius":"4px","background":"#FFF","width":"1200px"}'
                     v-model="activeName" type="border-card">
                <el-tab-pane label="制作方法" name="first">
                    <div v-html="detail.shangpinjianjie"></div>
                </el-tab-pane>
                <el-tab-pane label="评价" name="second">
                    <el-form class="add comment" :style='{"padding":"0px","margin":"12px 0 20px"}' :model="form"
                             :rules="rules" ref="form">


                        <el-select  v-model="form.pingfen" placeholder="请选择评分" >
                            <el-option
                        ...
                        <el-form-item class="item"
                                      :style='{"border":"1px solid #e6e6e6","boxShadow":"2px 4px 8px #eee,inset 0px 0px 100px 0px #f6f6f6","borderRadius":"8px","background":"#fff","display":"flex","width":"100%","height":"auto"}'
                                      label="评价" prop="content">
                            <el-input type="textarea" :rows="5" v-model="form.content" placeholder="请输入内容"></el-input>
                        </el-form-item>
                        <el-form-item class="btn"
                                      :style='{"width":"100%","padding":"0 0 0 20px","margin":"10px 0 0","height":"auto"}'>
                            <el-button
                                    :style='{"border":"0","cursor":"pointer","padding":"0","boxShadow":"1px 2px 8px #ccc","margin":"0 20px 0 0","color":"#fff","outline":"none","borderRadius":"4px","background":"radial-gradient(circle, rgba(43,224,232,1) 0%, rgba(0,173,181,1) 100%)","width":"128px","lineHeight":"40px","fontSize":"14px","height":"40px"}'
                                    type="primary" @click="submitForm('form')">立即提交
                            </el-button>
                            <el-button
                                    :style='{"border":"1px solid #bbb","cursor":"pointer","padding":"0","boxShadow":"1px 2px 3px #ccc","margin":"0 20px 0 0","color":"#999","outline":"none","borderRadius":"20px","background":"#fff","width":"128px","lineHeight":"40px","fontSize":"14px","height":"40px"}'
                                    @click="resetForm('form')">重置
                            </el-button>
                        </el-form-item>
                    </el-form>

                    <div v-if="infoList.length" :style='{"boxShadow":"0 0px 0px 0 rgba(0, 0, 0, .1)","padding":"15px"}'
                         class="comment">
                        <div :style='{"padding":"0px 0","margin":"0 0 8px","borderColor":"#ffeab2","alignItems":"center","borderWidth":"0 0 1px 0","width":"100%","borderStyle":"solid","height":"auto"}'
                            ...
                            <div :style='{"padding":"8px","margin":"8px 0px 0px","color":"#666","borderRadius":"4px","background":"none","lineHeight":"24px","fontSize":"14px","textIndent":"2em"}'
    ...
    export default {
        ...
        methods: {
            ...
            thumbsupOrCrazily(type) {
                ...
                this.$http.post('tiyuyongpin/update', this.detail).then(res => {
                });
            },
            ...
            storeup(type) {
                if (type == 1 && !this.isStoreup) {
                    this.storeupParams.name = this.title;
                    this.storeupParams.picture = this.detailBanner[0];
                    this.storeupParams.inteltype = this.detail.yongpinfenlei;
                    this.storeupParams.refid = this.detail.id;
                    this.storeupParams.type = type;
                    this.$http.post('storeup/add', this.storeupParams).then(res => {
                        if (res.data.code == 0) {
            ...
            addCart() {
                ...
                this.cart.goodtype = this.detail.yongpinfenlei;
            ...
            buyNow() {
                ...
                localStorage.setItem('orderGoods', JSON.stringify([{
                    ...
                    goodtype: this.detail.yongpinfenlei,
